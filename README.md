...
Autonomous coding pipelines that use [Claude Code](https://docs.anthropic.com/en/docs/claude-code) to fix bugs and implement tasks, opening draft merge requests on GitLab or pull requests on GitHub.
...
2. **Fix** — For real bugs, Claude Code runs in the repo with restricted tool access. It reads the codebase, finds the root cause, writes a fix, runs tests, commits to an `auto-fix/{id}` branch, and opens a draft MR/PR.
...
2. Claude reads the codebase, implements the task, runs tests, opens a draft MR/PR
...
cd bug-fixer
cp .env.example .env
# Edit .env — set GIT_PROVIDER, your token, and other keys
...
Host
  ...
  │           ├── iptables firewall: only git host, npm, Anthropic API

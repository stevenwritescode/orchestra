...
#
# Usage:
#   ANTHROPIC_API_KEY=sk-ant-... GITLAB_TOKEN=glpat-... ./worker.sh       (GitLab)
#   ANTHROPIC_API_KEY=sk-ant-... GITHUB_TOKEN=ghp-... GIT_PROVIDER=github ./worker.sh  (GitHub)
# =============================================================================
...
if [ "$GIT_PROVIDER" = "github" ]; then
    : "${GITHUB_TOKEN:?GITHUB_TOKEN is required when GIT_PROVIDER=github}"
    : "${GITHUB_REPO:?GITHUB_REPO is required when GIT_PROVIDER=github (e.g. owner/repo)}"
    CLI_TOOL="gh"
elif [ "$GIT_PROVIDER" = "gitlab" ]; then
    : "${GITLAB_TOKEN:?GITLAB_TOKEN is required when GIT_PROVIDER=gitlab}"
    : "${GITLAB_PROJECT_ID:?GITLAB_PROJECT_ID is required when GIT_PROVIDER=gitlab}"
    CLI_TOOL="glab"
else
    echo "GIT_PROVIDER must be 'github' or 'gitlab', got '${GIT_PROVIDER}'" >&2
    exit 1
fi
...
Please review carefully before merging."
    ...
    if [ "$GIT_PROVIDER" = "github" ]; then
        # Use GitHub API to create a draft pull request
        local response
        response=$(curl -s -w "\n%{http_code}" \
        ...
        if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
            ...
            log "Failed to create PR (HTTP ${http_code}): ${body}"
            return 1
        fi
    else
